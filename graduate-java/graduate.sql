...
(
    ...
    max_member   int unsigned default '200'             not null comment '群最大成员数（默�?00，可调整�?,
    join_type    tinyint      default 1                 not null comment '加群方式�?-需审核�?-免审核，3-仅邀�?,
    notice       text                                   null comment '群公�?,
    is_mute_all  tinyint      default 0                 not null comment '是否全员禁言�?-否，1-�?,
    status       tinyint      default 1                 not null comment '群状态：1-正常�?-解散�?-封禁',
    ext_info     json                                   null comment '扩展字段（如群标签、创建原因等�?,
    created_at   datetime     default CURRENT_TIMESTAMP not null comment '创建时间',
    updated_at   datetime     default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间'
)
    comment '群聊基础信息�? collate = utf8mb4_unicode_ci;
...
(
    ...
    role       tinyint  default 3                 not null comment '成员角色�?-群主�?-管理员，3-普通成�?,
    join_time  datetime default CURRENT_TIMESTAMP not null comment '加入时间',
    inviter_id varchar(64)                        null comment '邀请人ID（非邀请加入则为空�?,
    is_mute    tinyint  default 0                 not null comment '是否被禁言�?-否，1-是（仅群�?管理员可设置�?,
    is_quit    tinyint  default 0                 not null comment '是否退出：0-未退出，1-已退�?,
    ...
    constraint uk_group_user
        unique (group_id, user_id) comment '一个用户在一个群仅一条记�?
)
...
(
    ...
    session_type     tinyint                            not null comment '会话类型�?-单聊�?-群聊（核心区分场景）',
    sender_id        varchar(64)                        not null comment '发信人ID（用户ID/系统标识�?,
    receiver_id      varchar(64)                        null comment '接收人ID（单聊必填，群聊为空�?,
    message_type     tinyint                            not null comment '消息类型�?-文本�?-图片�?-视频�?-音频�?-文件�?-表情�?-系统通知',
    ...
    is_at_all        tinyint  default 0                 not null comment '是否@所有人�?-否，1-是（群聊核心需求）',
    ...
    duration         int unsigned                       null comment '语音/视频时长（秒），仅音�?视频消息有效',
    thumbnail_url    varchar(256)                       null comment '图片/视频缩略图URL（优化加载）',
    file_expired     tinyint  default 0                 not null comment '文件是否过期�?-有效�?-过期（避免加载失效文件）',
    send_time        datetime default CURRENT_TIMESTAMP not null comment '消息发送时�?,
    revoke_time      datetime                           null comment '撤回时间（非撤回消息为空�?,
    status           tinyint  default 1                 not null comment '核心状态：1-发送成功，2-已读�?-已撤回，4-已删除，5-发送失�?,
    operator_id      varchar(64)                        null comment '消息操作人ID（如管理员撤回消息时填管理员ID�?,
    ext_info         json                               null comment '轻量扩展字段（存储小众需求，避免加字段）',
    is_sensitive     tinyint  default 0                 not null comment '是否敏感消息�?-否，1-�?,
    is_deleted       tinyint  default 0                 not null comment '软删除标识：0-未删除，1-已删�?,
    created_at       datetime default CURRENT_TIMESTAMP not null comment '记录创建时间',
    updated_at       datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '记录更新时间',
    constraint uk_message_no
        unique (message_no) comment '唯一业务编号，防止重复消�?
)
...
(
    id                     bigint unsigned auto_increment comment '会话记录主键ID'
        primary key,
    session_id             varchar(64)                            not null comment '会话ID（和消息表一致：单聊=小ID_大ID；群�?group_群ID�?,
    session_type           tinyint                                not null comment '会话类型（和消息表一致：1-单聊�?-群聊�?,
    user_id                varchar(64)                            not null comment '【核心】所属用户ID（每个用户的每个会话都有一条记录）',
    target_id              varchar(64)                            not null comment '会话对方ID：单�?对方用户ID；群�?群ID',
    session_name           varchar(128)                           not null comment '会话名称：单�?对方昵称；群�?群名称（避免频繁查用�?群表�?,
    session_avatar         varchar(256)                           null comment '会话头像：单�?对方头像；群�?群头�?,
    ...
    last_message_time      datetime                               null comment '最后一条消息发送时�?,
    ...
    is_top                 tinyint      default 0                 not null comment '是否置顶�?-否，1-�?,
    is_mute                tinyint      default 0                 not null comment '是否免打扰：0-否，1-�?,
    is_hide                tinyint      default 0                 not null comment '是否隐藏会话�?-否，1-�?,
    is_deleted             tinyint      default 0                 not null comment '软删除标识：0-未删除，1-已删�?,
    ...
    constraint uk_user_session
        unique (user_id, session_id) comment '一个用户的一个会话仅一条记�?
)
...
(
    ...
    status     tinyint  default 1                 not null comment '用户状态：1-正常�?-禁用�?-注销',
    ...
    constraint uk_username
        unique (username) comment '用户名唯一，避免重复注�?
)
    comment '用户基础信息�? collate = utf8mb4_unicode_ci;
...
(
    ...
    status         tinyint  default 0                 not null comment '申请状态：0-待处�?1-已通过 2-已拒�?,
    reject_reason  varchar(255)                       null comment '拒绝原因（仅状态为2时填写）',
    created_at     datetime default CURRENT_TIMESTAMP not null comment '申请创建时间',
    updated_at     datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '申请更新时间',
