      ...
      <view class="modal-body">
        ...
        <view v-else class="result-card">
          ...
          <view class="result-action">
            ...
            <button v-else class="action-btn btn-add" :disabled="applying" @click="showRemarkInput = true">
              {{ applying ? '发送中...' : '+加好友' }}
            </button>
          </view>
        </view>
        ...
        <!-- 填写申请理由面板（点击加好友后展开） -->
        <view v-if="showRemarkInput && result && !result.isFriend && !result.isApplied" class="remark-panel">
          <text class="remark-label">验证信息（选填）</text>
          <textarea
            v-model="remark"
            class="remark-input"
            placeholder="请输入验证信息，让对方认证您的身份"
            maxlength="100"
            :auto-height="false"
          />
          <view class="remark-footer">
            ...
            <view class="remark-btns">
              <button class="action-btn btn-disabled" @click="showRemarkInput = false">取消</button>
              <button class="action-btn btn-add" :disabled="applying" @click="sendApply">
                {{ applying ? '发送中...' : '发送申请' }}
              </button>
            </view>
          </view>
...
const remark   = ref('')        // 申请备注
const showRemarkInput = ref(false) // 显示备注输入面板
...
.remark-label { font-size: 13px; color: #888; }
.remark-input {
  width: 100%;
  min-height: 64px;
  max-height: 100px;
...
.remark-count { font-size: 12px; color: #bbb; }
.remark-btns  { display: flex; gap: 8px; }
</style>
